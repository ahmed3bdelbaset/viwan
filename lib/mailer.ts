...
export async function sendContactNotification(data: ContactEmailData) {
  ...
  const rows = [
    ...
    { label: 'نوع المشروع', val: data.projectType || 'غير محدد' },
    { label: 'المساحة التقديرية', val: data.projectSize || 'غير محدد' },
    { label: 'الميزانية التقديرية', val: data.budget || 'غير محدد' },
    { label: 'مرحلة العمل الحالية', val: data.stage || 'غير محدد' },
